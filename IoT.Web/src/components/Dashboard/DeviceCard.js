    ...
    return (
        <div key={index} className="col-xs-12 col-sm-12 col-md-12 col-lg-4" style={{ paddingTop: 10 + 'px' }}>
            <div className="card text-black mb-3 h-100">
                <div className={deviceData?.status?.toLowerCase() === 'on' ? 'card-header bg-danger' : 'card-header bg-success'}>{deviceData.deviceName + ' - ' + deviceData?.deviceTypeName}</div>
                <div className="card-body">
                    <ol className="device-desc">
                        ...
                        <li key={index + "4"} title="Power" className={deviceData?.status?.toLowerCase() === 'on' ? 'text-danger' : 'text-success'}><i className={deviceData?.status?.toLowerCase() === 'on' ? 'text-success fas fa-power-off' : 'text-danger fas fa-power-off'}></i><span>{!common.hasValue(deviceData?.status) ? 'No Connected' : deviceData.status==="OFF"?"ON":"OFF"}</span></li>
