            ...
            <div className="card text-black mb-3 h-100">
                ...
                <div className="card-body">
                    <ol className="device-desc">
                        ...
                        <li key={index + "4"} title="Power" className={deviceData?.status?.toLowerCase() === 'on' ? 'text-success' : 'text-danger'}><i className={deviceData?.status?.toLowerCase() === 'on' ? 'text-success fas fa-power-off' : 'text-danger fas fa-power-off'}></i><span>{!common.hasValue(deviceData?.status) ? 'No Connected' : deviceData.status}</span></li>
                ...
                <div className="card-footer">
                    <DeviceControl deviceData={deviceData} devicePowerHandler={devicePowerHandler} ></DeviceControl>
                </div>
            </div>
        </div>
    )
